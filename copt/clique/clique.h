...
virtual	void set_graph(T* g_out){
	...
	T*	get_graph()					{return g;}				//added 28/9/17
...
//
// RETURNS global bound for the subgraph

	if(bbs.is_empty()){
		LOG_ERROR("Clique<ugraph>::compute_upper_bounds for an empty subgraph!");
		return 0;
	}

	int* rcol= get_root_coloring();
	const int NV=g->number_of_vertices(); 

	//compute oriented strong initial bounds
	InitColorUB c(*g);
	c.Compute_UB_enhanced_last(rcol, bbs);	//(1)

	//testing UB based on size
	//bbs.init_scan(bbo::NON_DESTRUCTIVE);
	//while(true){
	//	int v=bbs.next_bit();
	//	if(v==EMPTY_ELEM) break;
	...
	//compute absolute bound for the subgraph and store it as upper bound
	int max=0;
	bbs.init_scan(bbo::NON_DESTRUCTIVE);
	while(true){
		int v=bbs.next_bit();
